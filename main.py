...
# Startup event — ingest once here
@app.on_event("startup")
async def startup_event():
    global loaded
    ...
    try:
        ...
        run_ingestion()
        loaded = True
        logger.info("✅ Ingestion completed on startup!")
    except Exception as e:
        logger.error(f"❌ Ingestion failed on startup: {str(e)}")
        # Don't raise — let the server start anyway, /chat will return 503
...
def chat(req: QueryRequest):
    ...
    try:
        ...
        if not loaded:
            raise HTTPException(
                status_code=503,
                detail="Service is still initializing, please try again in a few seconds."
            )
        ...
        raise  # re-raise 503 cleanly without wrapping it
    except Exception as e:
        logger.error(f"Error in chat endpoint: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
